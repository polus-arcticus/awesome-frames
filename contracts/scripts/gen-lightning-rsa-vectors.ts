...
// src/grimoire/LightningRSA/LightningRSAAccount.yul — the 2-word (up to
// 512-bit) sibling of YoloRSA. Reuses YoloRSA's own keygen/sign/verify
...
// LightningRSAAccount.yul's calldata/storage layout widens YoloRSAAccount
// .yul's. Written to test/vectors/lightning-rsa-vectors.json, read back by
// test/js/LightningRSA.test.ts.
//
...
// measurements — see LightningRSAAccount.yul's header.
//
// IMPORTANT: same rule as gen-yolo-rsa-vectors.ts — these keypairs are TEST
// FIXTURES ONLY, fine to commit. A real deployed LightningRSA wallet must
// generate its own keypair locally, with `d` never written to any
// committed file.
//
